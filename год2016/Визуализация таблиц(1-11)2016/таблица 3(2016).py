...
xls_file = pd.read_excel('FILE76_01.01.2016_178_178.xls', 'Раздел3', index_col=None)
os.chdir('C:\\Users\\Кирилл\\Downloads\\PythonProject\\год2016')
xls_file[13:].to_csv('Третья часть3.csv', encoding='utf-8',sep=';',index=False)
df=pd.read_csv('Третья часть3.csv',sep=';', comment='#',index_col=None)
df = df.fillna('')
df = df.rename(columns={'Раздел 3.Льготы по отдельным администрируемым доходам': 'ПОКАЗАТЕЛЬ', 'Unnamed: 2': '2016 год','Unnamed: 3':'В % к предыдущему году'})
del df['Unnamed: 1']
...
df.to_html('Таблица_3(2016).html',index=False)