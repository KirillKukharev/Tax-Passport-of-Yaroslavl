...
xls_file = pd.read_excel('FILE76_01.01.2016_178_178.xls', 'Раздел5', index_col=None)
os.chdir('C:\\Users\\Кирилл\\Downloads\\PythonProject\\год2016')
xls_file[16:].to_csv('Пятая часть3.csv', encoding='utf-8',sep=';',index=False)
df=pd.read_csv('Пятая часть3.csv',sep=';', comment='#',index_col=None)
df = df.fillna('')
for i in range(3,13):
    k='Unnamed: '+ str(i)
...
df.to_html('Таблица_5(2016).html',index=False)
